...
const Options = () => {
  ...
  return (
    <div
      ...
      <nav
        className={`w-48 border-r ${isDarkMode ? 'border-slate-700 bg-slate-800/80' : 'border-white/20 bg-[#0D9488]/10'} backdrop-blur-sm`}>
        <div className="p-4">
              ...
              <li key={item.id}>
                <Button
                  ...
                  className={`flex w-full items-center space-x-2 rounded-lg px-4 py-2 text-left text-base 
                    ${
                      activeTab !== item.id
                        ? `${isDarkMode ? 'bg-slate-700/70 text-gray-300 hover:text-white' : 'bg-[#0D9488]/15 font-medium text-gray-700 hover:text-white'} backdrop-blur-sm`
                        : `${isDarkMode ? 'bg-teal-800/50' : ''} text-white backdrop-blur-sm`
                    }`}>
